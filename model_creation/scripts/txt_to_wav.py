...
def load_txt():
    ...
    for cls in classes:
        with open(f"TinyML-Rain-Detection/model_creation/model_data/raw_data/{cls}.txt", 'r') as f:
            data_str = f.read()
        ...
        write(f"TinyML-Rain-Detection/model_creation/model_data/wav_data/{cls}.wav", sample_rate, data.astype(np.int16))


def crop_file():
    classes = ["heavy_rain", "light_rain", "no_rain", "medium_rain"]
    for cls in classes:
        sr, signal = wavfile.read(f"TinyML-Rain-Detection/model_creation/model_data/wav_data/{cls}.wav")
        ...
        wavfile.write(f"TinyML-Rain-Detection/model_creation/model_data/wav_data_cropped/{cls}.wav", sample_rate, cropped_signal)
